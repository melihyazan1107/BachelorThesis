#!/bin/bash
#SBATCH --job-name=rq1_train_mobilenetv2
#SBATCH --output=/home/yazanme/BachelorThesis/BachelorThesis/slurm/rq1/logs/rq1_train_mobilenetv2_%j.log
#SBATCH --error=/home/yazanme/BachelorThesis/BachelorThesis/slurm/rq1/logs/rq1_train_mobilenetv2_%j.log
...
# =========================================
# 3. AUSFÜHRUNG RQ1 TRAINING (${MODEL})
# =========================================
...
TRAIN_CONFIG="${REPO_ROOT}/src/yaml/rq1/rq1_train_${MODEL}.yaml"
...
if [ ${MISSING} -gt 0 ]; then
    echo "ABBRUCH: ${MISSING} von 15 RQ1-Label-Dateien fehlen (rate 0.0 braucht keine)." >&2
    exit 1
fi
...
echo "Ausgabe:          ${REPO_ROOT}/experiments/RQ1/${MODEL}"
echo "Preflight ok: 3 HDF5-Splits + 15 Label-Tensoren vorhanden."
echo ""
...
EXP_DIR="${REPO_ROOT}/experiments/RQ1/${MODEL}"
if [ -d "${EXP_DIR}" ]; then
    echo ""
    ...
    if [ -f "${EXP_DIR}/summary_metrics.csv" ]; then
        echo ""
        echo "summary_metrics.csv:"
        cat "${EXP_DIR}/summary_metrics.csv"
    fi
fi
