#!/bin/bash
#SBATCH --job-name=rq1_train_convnext_tiny
#SBATCH --output=/home/yazanme/BachelorThesis/BachelorThesis/slurm/rq1/logs/rq1_train_convnext_tiny_%A_%a.log
#SBATCH --error=/home/yazanme/BachelorThesis/BachelorThesis/slurm/rq1/logs/rq1_train_convnext_tiny_%A_%a.log
#SBATCH --array=0-2
...
# =========================================
# 3. AUSFÜHRUNG RQ1 TRAINING (${MODEL}, Seed ${SEED})
# =========================================
...
TRAIN_CONFIG="${REPO_ROOT}/src/yaml/rq1/rq1_train_${MODEL}_seed${SEED}.yaml"
...
for r in 0.01 0.02 0.05 0.10 0.20; do
    lbl="${PROCESSED}/poisoned/RQ1/seed_${SEED}/labels_${r}.pt"
    if [ ! -f "${lbl}" ]; then
        echo "FEHLT: ${lbl}" >&2
        MISSING=$((MISSING + 1))
    fi
...
if [ ${MISSING} -gt 0 ]; then
    echo "ABBRUCH: ${MISSING} von 5 RQ1-Label-Dateien fuer Seed ${SEED} fehlen (rate 0.0 braucht keine)." >&2
    exit 1
fi
...
echo "Ausgabe:          ${REPO_ROOT}/experiments/RQ1/${MODEL}/seed_${SEED}"
echo "Preflight ok: 3 HDF5-Splits + 5 Label-Tensoren fuer Seed ${SEED} vorhanden."
echo ""
...
SEED_DIR="${REPO_ROOT}/experiments/RQ1/${MODEL}/seed_${SEED}"
if [ -d "${SEED_DIR}" ]; then
    echo ""
    echo "Abgeschlossene Laeufe fuer Seed ${SEED} (erwartet: 6 Raten):"
    find "${SEED_DIR}" -mindepth 1 -maxdepth 1 -type d -name 'rate_*' | sort
    echo "Anzahl: $(find "${SEED_DIR}" -mindepth 1 -maxdepth 1 -type d -name 'rate_*' | wc -l)"
fi
